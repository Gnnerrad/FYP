...
public class Player {
	...
	public Card playCard(ArrayList<Card> legalMoves, Card otherPlayersCard) {
		...
		double[] currentOutput, bestOutput = new double[NNSettings.outputSize],
				bestInput = new double[NNSettings.inputSize];
		Card bestCard = null;
	...
	public int chooseGamemode(ArrayList<Integer> gameModesAvailable) {
		int bestGameMode = -1;
		double currentResult, bestResult = -100;
		double[] results = new double[NNSettings.outputSize], bestOutput = new double[NNSettings.outputSize],
				bestInput = new double[216];
	...
	private double calculateResult(double[] output) {
		...
		return (((2 * output[0] + output[2]) / (2 * output[4] + 1))
				+ ((2 * output[1] + output[2]) / (2 * output[3] + 1)))
				- (((2 * output[3] + output[2]) / (2 * output[1] + 1))
						+ ((2 * output[4] + output[2]) / (2 * output[0] + 1)));
	}
