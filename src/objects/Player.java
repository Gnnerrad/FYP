...
public class Player {
	...
	public Card playCard(ArrayList<Card> legalMoves, Card otherPlayersCard) {
		...
		for (Card handCard : legalMoves) {
			...
			currentResult = ((2 * currentOutput[0] + currentOutput[1]) / (2 * currentOutput[2] + 1))
					- ((2 * currentOutput[2] + currentOutput[1]) / (2 * currentOutput[0] + 1));
	...
	public int chooseGamemode(ArrayList<Integer> gameModesAvailable) {
		int bestGameMode = -1;
		double currentResult, bestResult = -100;
		double[] results = new double[3], currentOutput = new double[3], bestOutput = new double[3],
				bestInput = new double[214];
