...
public class GameDriver {
	...
	for (int x = 0; x < 4; x++) {
	    switch (x) {
	    case 0:
		NNSettings.nn = "lib/Epoch lr 0.001.nnet";
		NNSettings.learningRate = 0.001;
		break;
	    case 1:
		NNSettings.nn = "lib/Epoch lr 0.0001.nnet";
		NNSettings.learningRate = 0.0001;
		break;
	    case 2:
		NNSettings.nn = "lib/Epoch lr 0.00001.nnet";
		NNSettings.learningRate = 0.00001;
		break;
	    // case 3:
		...
		testGame(NNSettings.nn, 10, "data/Epoch lr " + NNSettings.learningRate + " rmse", epoch);
	...
	if (trainFile) {
	    for (int i = 0; i < 50; i++) {
		playGame(false, false, NNSettings.randNN, NNSettings.randNN, null, randomPlayGames);
	    }
	    randomPlayGames.writeFile(NNSettings.player1GameFile, NNSettings.player2GameFile);
	} else {
	    for (int i = 0; i < 50; i++) {
		playGame(false, false, NNSettings.randNN, NNSettings.randNN, "data\100 deck", randomPlayGames);
	    }
