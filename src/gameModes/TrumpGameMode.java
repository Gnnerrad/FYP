...
//		return (player1Tricks > player1Tricks) ? true : false;
	...
	public void chooseTrumpSuit(String suit){
		switch (suit) {
			...
			case "diamonds":
				...
				break;
			default:
				System.out.println("The suit '" + suit + "' is not recognised.\nPlease choose one of: clubs, spades, hearths, diamonds");
	...
	private boolean calculateWinner(Card card1, Card card2){
		if(card1.getSuit().getCardSuit() == trumpSuit && card2.getSuit().getCardSuit() != trumpSuit){
			return true; //player1 wins by trump
		}
		else if(card1.getSuit().getCardSuit() != trumpSuit && card2.getSuit().getCardSuit() == trumpSuit){
			return false; //player2 wins by trump
		}
		else{
			return (card1.getValue().getCardValue() > card2.getValue().getCardValue());
		}
	}
