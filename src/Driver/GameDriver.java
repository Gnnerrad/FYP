...
public class GameDriver {
	...
	public static void main(String[] args) {
		...
		driver.driver();
		System.out.println(NNSettings.gmC + " + " + NNSettings.disC + " + " + NNSettings.acnxtC + " + "
				+ NNSettings.whoC + " + " + NNSettings.playC + " = "
				+ (NNSettings.gmC + NNSettings.disC + NNSettings.acnxtC + NNSettings.whoC + NNSettings.playC));
	...
	private void driver() {
		...
		for (int x = 0; x < 6; x++) {
			switch (x) {
			case 0:
				NNSettings.learningRate = 0.0005;
				NNSettings.nn = "Data 2.0/Self 100 lr 0.0005.nnet";
				break;
			case 1:
				NNSettings.learningRate = 0.00025;
				NNSettings.nn = "Data 2.0/Self 100 lr 0.00025.nnet";
				break;
			case 2:
				NNSettings.learningRate = 0.0001;
				NNSettings.nn = "Data 2.0/Self 100 lr 0.0001.nnet";
				break;
			case 3:
				NNSettings.learningRate = 0.000075;
				NNSettings.nn = "Data 2.0/Self 100 lr 0.000075.nnet";
				break;
			case 4:
				NNSettings.learningRate = 0.00005;
				NNSettings.nn = "Data 2.0/Self 100 lr 0.00005.nnet";
				break;
			}
			...
			for (int i = 0; i <= 10000; i++) {
				...
				gd.clear();
				if (i % 50 == 0)
					testGame(NNSettings.nn, 10, "Data 2.0/Selfv100 lr " + NNSettings.learningRate + " rmse", i);
	...
	private void testGame(String NN, int games, String filePath, int writeIndex) {
		...
		try (BufferedWriter out = new BufferedWriter(new FileWriter(filePath, true))) {
			out.write(writeIndex + "\t" + Math.sqrt(gameModeRmse / gameModeCount) + "\t"
					+ Math.sqrt(discard6Rmse / discard6Count) + "\t" + Math.sqrt(acceptOrNextRmse / acceptOrNextCount)
					+ "\t" + Math.sqrt(playCardRmse / playCardCount) + "\n");
		} catch (Exception e) {

		}
	}

	private void playGame(boolean selfPlay, boolean printOutcome, String player1NN, String player2NN, String DeckFile,
			GameData gm) {
		player1Rounds = 0;
		player2Rounds = 0;
		...
		if (printOutcome)
			System.out.println(
					player1WinsB + "-" + player1WinsS + ":" + drawCount + ":" + player2WinsS + "-" + player2WinsB);
